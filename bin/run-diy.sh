#!/bin/bash
#
# Executes a HipG program on a cluster using prun.
...
PRUNOPT="-t 60:00 -v -$SLOTS -np $NPROC"
if [ -n "$OUT" ]; then
	PRUNOPT="$PRUNOPT -o $OUT"
fi
#PRUNOPT="$PRUNOPT -sge-script $PRUNSCRIPT"
#PRUNOPT="$PRUNOPT -native '-R y -p -1'"
...
# Command to run in parallel (eg submit to SGE)
#COMMAND="prun -native '-ar 94' $PRUNOPT $JAVACOMMAND"
#COMMAND="prun $PRUNOPT $JAVACOMMAND"
COMMAND="ssh root@diy-3-7 \"cd HipG-2.0; $JAVACOMMAND\""

echo "Running $COMMAND"
#prun -native '-ar 94' $PRUNOPT $JAVACOMMAND
#prun $PRUNOPT $JAVACOMMAND

if [ $POOLSIZE -ge 1 ]; then
	ssh root@diy-3-7 "cd HipG-2.0; $JAVACOMMAND" &
fi
if [ $POOLSIZE -ge 2 ]; then
	ssh root@diy-3-10 "cd HipG-2.0; $JAVACOMMAND" &
fi
if [ $POOLSIZE -ge 3 ]; then
	ssh root@diy-3-11 "cd HipG-2.0; $JAVACOMMAND" &
fi
if [ $POOLSIZE -ge 4 ]; then
	ssh root@diy-3-13 "cd HipG-2.0; $JAVACOMMAND" &
fi
if [ $POOLSIZE -ge 5 ]; then
	ssh root@diy-3-7 "cd HipG-2.0; $JAVACOMMAND" &
fi
if [ $POOLSIZE -ge 6 ]; then
	ssh root@diy-3-10 "cd HipG-2.0; $JAVACOMMAND" &
fi
if [ $POOLSIZE -ge 7 ]; then
	ssh root@diy-3-11 "cd HipG-2.0; $JAVACOMMAND" &
fi
if [ $POOLSIZE -ge 8 ]; then
	ssh root@diy-3-13 "cd HipG-2.0; $JAVACOMMAND" &
fi
