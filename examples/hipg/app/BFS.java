...
public class BFS {
	...
	public static class BFSSynch extends Synchronizer {
		...
		public void run() {
			...
			avgBFS = EstAvgBFS(0.0) / (double) Runtime.getPoolSize();
			globalVisited = GlobalVisited(0);
		}

		@Reduce
		public double EstAvgBFS(double s) {
			long sum = 0;
			int count = 0;
			...
			return s + (count == 0 ? 0.0 : (double) sum / (double) count);
		}
	...
	public static void main(String[] args) throws GraphCreationException {
		if (args.length < 2) {
			System.err.println(BFS.class.getName() + " <graph> [ <root id> <root owner> ]");
		...
		// print results
		print("Average BFS = " + MathUtils.round(sssp.avgBFS) + ", visited " + sssp.globalVisited + " nodes");
